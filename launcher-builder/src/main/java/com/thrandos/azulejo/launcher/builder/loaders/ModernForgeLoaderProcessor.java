...
import com.thrandos.azulejo.launcher.model.loader.profiles.ModernForgeInstallProfile;
				...
				if (finalJars != null) {
					...
					for (Library lib : result.getLoaderLibraries()) {
						if (lib.matches(libraryName)) {
							...
							log.info("Setting generated flag on library '%s'".formatted(lib.getName()));
							break;
						}
					}
				}

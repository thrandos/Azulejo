...
import com.thrandos.azulejo.launcher.builder.BuilderUtils;
import com.thrandos.azulejo.launcher.model.loader.FabricMod;
import com.thrandos.azulejo.launcher.model.loader.QuiltMod;
import com.thrandos.azulejo.launcher.model.loader.Versionable;
import com.thrandos.azulejo.launcher.model.minecraft.Library;
import com.thrandos.azulejo.launcher.model.minecraft.MavenName;
	...
	public LoaderResult process(File loaderJar, Manifest manifest, ObjectMapper mapper, File baseDir) throws IOException {
		...
		try {
			...
			if (modEntry != null) {
				...
				log.warning("%s loader has no '%s' file, is it really a %s Loader jar?".formatted(
                        variant.friendlyName, variant.modJsonName, variant.friendlyName));
				return null;
			}

			log.info("Downloading %s metadata...".formatted(variant.friendlyName));
			URL metaUrl = HttpRequest.url(
                    variant.metaUrl.formatted(manifest.getGameVersion(), loaderMod.getVersion()));
			...
			log.warning("HTTP request to %s metadata API was interrupted!".formatted(variant.friendlyName));

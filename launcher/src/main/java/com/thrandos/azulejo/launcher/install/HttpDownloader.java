...
public class HttpDownloader implements Downloader {
    ...
    public synchronized File download(@NonNull List<URL> urls, @NonNull String key, long size, String name) {
        ...
        String hash = hf.hashString(Strings.nullToEmpty(key) + urls.getFirst(), Charsets.UTF_8).toString();
    ...
    public synchronized String getStatus() {
        ...
        if (running.size() == 1) {
            return tr("downloader.downloadingItem", running.getFirst().getName()) +
                    "\n" + running.getFirst().getStatus() +
                    "\n" + failMessage;
    ...
    public class HttpDownloadJob implements Runnable, ProgressObservable {
        ...
        private void download() throws IOException, InterruptedException {
            ...
            if (!tempFile.renameTo(destFile)) {
                throw new IOException("Failed to rename %s to %s".formatted(tempFile, destFile));
            }
        }

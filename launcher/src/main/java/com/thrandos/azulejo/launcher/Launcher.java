...
public final class Launcher {
    ...
    /** 
     * ok so this class was generated by Claude and I don't know what it does but the AI seems really proud of it
     * 
     * so I guess I'm keeping it?
    */

    private void initializeDirectories() {
        log.info("Initializing launcher directories in: " + baseDir.getAbsolutePath());
        ...
        File[] dirs = {
            ...
            getTemporaryDir(),
            getInstallerDir(),
            getLauncherBinariesDir(),
            getLibrariesDir(),
            getVersionsDir(),
            assets.getDir(),
            new File(assets.getDir(), "indexes"),
            new File(assets.getDir(), "objects")
        };
        ...
        for (File dir : dirs) {
            if (!dir.exists()) {
                if (dir.mkdirs()) {
                    ...
                    log.warning("Failed to create directory: " + dir.getAbsolutePath());
                }
            }
        }
    }
     ...
     * <p>
     * When running from a jpackage app-image the JAR lives inside the
     * {@code app/} sub-folder, and the {@code .exe} sits one level above it.
     * We want the base directory to be next to the {@code .exe}, not inside
     * {@code app/}.
     *
     * @return the application root directory, or the current directory as fallback
     */
    private static File getJarDirectory() {
        // jpackage sets this system property to the path of the native launcher (.exe)
        String appPath = System.getProperty("jpackage.app-path");
        if (appPath != null && !appPath.isEmpty()) {
            File exe = new File(appPath);
            if (exe.isFile()) {
                log.info("Detected jpackage app-path: " + exe.getAbsolutePath());
                return exe.getParentFile().getAbsoluteFile();
            }
        }

        // Fallback: locate via the code-source (works for plain JAR / IDE)
        try {
            String jarPath = Launcher.class.getProtectionDomain().getCodeSource().getLocation().toURI().getPath();
            File jarFile = new File(jarPath);

            if (jarFile.isFile()) {
                File parent = jarFile.getParentFile().getAbsoluteFile();

                // If the JAR is inside an "app" folder created by jpackage,
                // move one level up to sit next to the .exe
                if ("app".equalsIgnoreCase(parent.getName())) {
                    log.info("JAR is inside jpackage 'app' folder — using parent: " + parent.getParent());
                    return parent.getParentFile().getAbsoluteFile();
                }
                return parent;
            } else if (jarFile.isDirectory()) {
                // Running from exploded classes (IDE / dev mode)
                return jarFile.getAbsoluteFile();
            }
        } catch (Exception e) {
            log.warning("Could not determine JAR/EXE location: " + e.getMessage());
        }

        // Last resort: use the directory the process was launched from

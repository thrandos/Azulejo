...
package com.thrandos.azulejo.launcher.launch.runtime;
...
public final class JavaRuntimeFinder {
    ...
    private static PlatformRuntimeFinder getRuntimeFinder(Environment env) {
        return switch (env.getPlatform()) {
            case WINDOWS -> new WindowsRuntimeFinder();
            case MAC_OS_X -> new MacRuntimeFinder();
            case LINUX -> new LinuxRuntimeFinder();
            default -> null;
        };
    }
}

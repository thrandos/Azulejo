...
import com.thrandos.azulejo.launcher.model.loader.LoaderSubResolver;
...
public class ProcessorTask implements InstallTask {
	...
	public void execute(Launcher launcher) throws Exception {
		...
		if (mainClass == null || mainClass.isEmpty()) {
			throw new RuntimeException("Processor jar file '%s' has no main class!".formatted(processor.getJar()));
		}
		...
		for (String libraryName : processor.getClasspath()) {
			...
			if (!libraryFile.exists()) {
				throw new RuntimeException("Missing library '%s' for processor '%s'".formatted(
		...
		log.info("Running processor '%s' with %d args".formatted(processor.getJar(), programArgs.size()));
		...
		if (!outputs.isEmpty()) {
			...
			for (Map.Entry<String, String> output : outputs.entrySet()) {
				...
				if (!artifact.exists()) {
					throw new RuntimeException("Artifact '%s' missing".formatted(output.getKey()));
				}

				if (!FileUtils.getShaHash(artifact).equals(output.getValue())) {
					log.warning("Invalid hash, expected " + output.getValue());
					throw new RuntimeException("Artifact '%s' has invalid hash!".formatted(output.getKey()));
				}
